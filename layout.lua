...
do
    ...
    OnHealthValueChanged = function(oldBar, curr)
        ...
        -- 1. FETCH REAL VALUES FROM YOUR DATABASE
        frame.health.min, frame.health.max = oldBar:GetMinMaxValues()
        frame.health.curr = curr
        
        -- Use MobHealth3 to override percentages with real numbers
        if MobHealth3 and MobHealth3.GetUnitHealth then
            local c, m, found
            if frame.target then
                c, m, found = MobHealth3:GetUnitHealth("target")
            elseif frame.name.text then
                -- Safely get the level string from the nameplate
                local levelText = frame.level:GetText() or "0"
                
                -- Strip any non-number characters (removes Boss, ?, +, etc)
                local levelStr = string.gsub(levelText, "%D+", "")
                
                -- Convert to a number for the database search
                local level = tonumber(levelStr) or 0
                
                -- Fetch from your 8,000-mob database
                c, m, found = MobHealth3:GetUnitHealth(nil, curr, 100, frame.name.text, level)
            end
        ...
        frame.health.percent = (frame.health.max > 0) and (frame.health.curr / frame.health.max) * 100 or 0
        
        -- Update the actual bar scale to reflect real numbers
        frame.health:SetMinMaxValues(0, frame.health.max)
        frame.health:SetValue(frame.health.curr)

        -- 2. APPLY TEXT PATTERN
        -- select correct health display pattern based on your options
        ...
        -- parse pattern into big/sml display strings
        ...
        for _, rule in ipairs(rules) do
            ...
            elseif condition == '<=' or condition == '=<' then
                ...
                condition = true -- Default if no condition provided
            end
            ...
            if condition then
                ...
                elseif display == 'c' then
                    -- Ensure both values are populated as strings
                    big = kui.num(frame.health.curr)
                    sml = kui.num(frame.health.max)
                end
                break
            end
        end

        -- Update the font strings on the nameplate
        if big and sml then
            -- This creates the "Current / Total" format on a single line
            frame.health.p:SetText(big .. " / " .. sml)
            if frame.health.mo then frame.health.mo:SetText('') end
        else
            frame.health.p:SetText(big or '')
            if frame.health.mo then frame.health.mo:SetText(sml or '') end
        end
    end
end
...
local function OnFrameShow(self)
    ...
    -- FIX: Force immediate combat check when the frame shows
    if addon.db.profile.general.combat then
